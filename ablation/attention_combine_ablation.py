...
def attention_combine_ablation():
    """注意力机制与组合融合方式的模态消融实验"""
    # 定义要测试的模态组合
    modality_combinations = [
        # {'use_text': True, 'use_image': False, 'text_dim': 128, 'dropout': 0.15, 'learning_rate': 8e-05},  # 仅文本
        # {'use_text': False, 'use_image': True, 'text_dim': 128, 'dropout': 0.15, 'learning_rate': 8e-05},  # 仅图像
        {'use_text': True, 'use_image': True, 'text_dim': 128, 'dropout': 0.15, 'learning_rate': 8e-05}    # 文本+图像
    ]
    ...
    for idx, modality in enumerate(modality_combinations):
        ...
        # 根据参数生成实验名称
        exp_name = f"attention_combine_text{modality['use_text']}_image{modality['use_image']}_textdim{modality['text_dim']}_dropout{modality['dropout']}_lr{modality['learning_rate']}"
        ...
        # 创建日志目录
        log_dir = os.path.join(script_dir, "../logs/ablation_attention_combine_logs")
