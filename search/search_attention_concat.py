...
def search_attention_concat_params():
    """测试attention_concat融合方式的超参数搜索"""
    # 定义要搜索的超参数空间
    param_grid = {
        'text_dim': [128, 256],  # 文本维度
        'image_dim': [128, 256],  # 图像维度
        'dropout': [0.15],  # dropout率
        'learning_rate': [8e-5],  # 学习率
        'feature_fusion': ['attention_concat']  # 固定使用attention_concat融合方式
    }
    ...
    # 遍历所有参数组合
    total_combinations = len(param_grid['text_dim']) * len(param_grid['image_dim']) * \
                        len(param_grid['dropout']) * len(param_grid['learning_rate'])
    current_combination = 0

    for text_dim in param_grid['text_dim']:
        for image_dim in param_grid['image_dim']:
            for dropout in param_grid['dropout']:
                for lr in param_grid['learning_rate']:
                    current_combination += 1
                    print(f"\n正在测试组合 {current_combination}/{total_combinations}")
                    print(f"参数: text_dim={text_dim}, image_dim={image_dim}, "
                         f"dropout={dropout}, lr={lr}, fusion=attention_concat")
                    
                    # 根据参数生成实验名称
                    exp_name = f"attention_concat_textdim{text_dim}_imagedim{image_dim}_dropout{dropout}_lr{lr}"
                    ...
                    # 创建日志目录
                    log_dir = os.path.join(script_dir, "../logs/concat_search_logs")
