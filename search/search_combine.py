...
def search_combine_params():
    """测试combine融合方式的超参数搜索"""
    # 定义要搜索的超参数空间
    param_grid = {
        'text_dim': [128, 256, 512],  # 文本维度
        'dropout': [0.1, 0.25],  # dropout率
        'learning_rate': [1e-4, 1e-5],  # 学习率
        'feature_fusion': ['combine']  # 固定使用combine融合方式
    }
        ...
        for dropout in param_grid['dropout']:
            for lr in param_grid['learning_rate']:
                ...
                print(f"参数: text_dim={text_dim}, "
                     f"dropout={dropout}, lr={lr}, fusion=combine")
                
                # 根据参数生成实验名称
                exp_name = f"combine_textdim{text_dim}_dropout{dropout}_lr{lr}"
                ...
                # 创建日志目录
                log_dir = os.path.join(script_dir, "../logs/combine_search_logs")
