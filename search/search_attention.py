...
def search_attention_params():
    """测试attention融合方式的超参数搜索"""
    # 定义要搜索的超参数空间
    param_grid = {
        'text_dim': [128, 256, 512],  # 文本维度
        'dropout': [0.1, 0.2],  # dropout率
        'learning_rate': [2e-5, 5e-6],  # 学习率
        'feature_fusion': ['attention']  # 固定使用attention融合方式
    }
        ...
        for dropout in param_grid['dropout']:
            for lr in param_grid['learning_rate']:
                ...
                print(f"参数: text_dim={text_dim}, "
                     f"dropout={dropout}, lr={lr}, fusion=attention")
                
                # 根据参数生成实验名称
                exp_name = f"attention_textdim{text_dim}_dropout{dropout}_lr{lr}"
                ...
                # 创建日志目录
                log_dir = os.path.join(script_dir, "../logs/attention_search_logs")
